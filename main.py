...
async def get_name(message: types.Message):
    ...
    logging.info(f"Користувач { message.from_user.first_name} вказав ім'я: {message.text}")
...
async def get_phone(message: types.Message):
    if message.contact and message.chat.id in user_data:
        ...
        logging.info(f"Користувач { message.from_user.first_name} надіслав номер телефону: {message.contact.phone_number}")
...
async def manual_phone_input(message: types.Message):
    ...
    if validate_phone_number(phone_number):
        ...
        logging.info(f"Користувач {message.from_user.first_name} вказав номер телефону вручну: {phone_number}")
        await message.answer("Дякую! Тепер натисни 'Поїхали! 🚀', щоб почати опитування.", reply_markup=get_start_keyboard())
    else:
...
async def send_welcome(message: types.Message):
    ...
    logging.info(f"Надіслано інформацію про користувача {message.from_user.first_name} - {message.from_user.username} адміністратору.")
...
async def start_questions(message: types.Message):
    if user_data[message.chat.id]['name'] is None or user_data[message.chat.id]['phone'] is None:
        ...
        logging.warning(f"Користувач {message.from_user.first_name} намагається почати опитування без імені або номера телефону.")
        return
    user_answers[message.chat.id] = []
    await message.answer(questions[0], reply_markup=get_keyboard())
    logging.info(f"Користувач {message.from_user.first_name} починає опитування.")
...
async def handle_answer(message: types.Message):
    ...
    if current_question_index < len(questions):
        ...
        logging.info(f"Користувач {message.from_user.first_name} відповідає на питання {current_question_index + 1}: {message.text.lower()}")
        if current_question_index + 1 < len(questions):
            ...
            logging.info(f"Користувач {message.from_user.first_name} завершив опитування.")
